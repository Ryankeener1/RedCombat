...
namespace RedCombat
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        enum Maps { Day, Dawn, Night, Dusk};
        Maps MapSelect;

        Texture2D Day, Night, Dawn, Dusk;
        Color ScreenColor;

        Rectangle MapSelector;

        int Plane1Cooldown, Plane1Active, Plane2Cooldown, Plane2Active, Plane1Ability, Plane2Ability;
        ...
        protected override void LoadContent()
        {
            ...
            heartText = Content.Load<Texture2D>("Heart");
            ...
            Dusk = Content.Load<Texture2D>("Dusk");
            Dawn = Content.Load<Texture2D>("Dawn");
                        ...
                        switch (Team1Menu)
                        {
                            ...
                            case (PlaneMenu)5:
                                ...
                                if (gPlayer1Plane.Buttons.A == ButtonState.Pressed)
                                {
                                    Ready1Bool = true;
                                    Ready1Color = Team1;
                                    Player1Plane = new Plane(PlanesTexts[(int)currentplane1], new Rectangle(100, 100, Plane1Width, Plane1Height), Team1, new Vector2(Plane1Speed, Plane1Speed), Plane1Reload, Plane1Bullets, Bullet, PlayerIndex.One, Plane1Active, Plane1Cooldown, (Plane.Abilities)Plane1Ability);
                                }
                        ...
                        switch (Team2Menu)
                        {
                            ...
                            case (PlaneMenu)5:
                                ...
                                if (gPlayer2Plane.Buttons.A == ButtonState.Pressed)
                                {
                                    Ready2Bool = true;
                                    Ready2Color = Team2;
                                    Player2Plane = new Plane(PlanesTexts[(int)currentplane2], new Rectangle(100, 100, Plane2Width, Plane2Height), Team2, new Vector2(Plane2Speed, Plane2Speed), Plane2Reload, Plane2Bullets, Bullet, PlayerIndex.Two, Plane2Active, Plane2Cooldown, (Plane.Abilities)Plane2Ability);
                ...
                case ((MenuScreen)2):
                    {
                        ...
                        if(gPlayer1Plane.DPad.Down == ButtonState.Pressed && oldgPlayer1Plane.DPad.Down == ButtonState.Released)
                        {
                            MapSelect++;
                        }

                        if (gPlayer1Plane.DPad.Up == ButtonState.Pressed && oldgPlayer1Plane.DPad.Up == ButtonState.Released)
                        {
                            MapSelect--;
                        }
                        ...
                        switch (MapSelect)
                        {
                            case ((Maps)0):
                                {
                                    MapSelector = new Rectangle(400, 95, 280, 60);

                                    if(gPlayer1Plane.Buttons.A == ButtonState.Pressed && oldgPlayer1Plane.Buttons.A == ButtonState.Released)
                                    {
                                        ScreenColor = mapColors[0];
                                    }
                                }
                                break;
                            ...
                            case ((Maps)1):
                                {
                                    MapSelector = new Rectangle(400, 245, 280, 60);

                                    if (gPlayer1Plane.Buttons.A == ButtonState.Pressed && oldgPlayer1Plane.Buttons.A == ButtonState.Released)
                                    {
                                        ScreenColor = mapColors[1];
                                    }
                                }
                                break;
                            ...
                            case ((Maps)2):
                                {
                                    MapSelector = new Rectangle(400, 395, 280, 60);

                                    if (gPlayer1Plane.Buttons.A == ButtonState.Pressed && oldgPlayer1Plane.Buttons.A == ButtonState.Released)
                                    {
                                        ScreenColor = mapColors[2];
                                    }
                                }
                                break;
                            ...
                            case ((Maps)3):
                                {
                                    MapSelector = new Rectangle(400, 545, 280, 60);

                                    if (gPlayer1Plane.Buttons.A == ButtonState.Pressed && oldgPlayer1Plane.Buttons.A == ButtonState.Released)
                                    {
                                        ScreenColor = mapColors[3];
                                    }
                                }
                                break;
                        ...
                        if (gPlayer1Plane.Buttons.Start == ButtonState.Pressed && oldgPlayer1Plane.Buttons.Start == ButtonState.Released)
                        {
                            menuselect++;
                        }

                        if(gPlayer1Plane.Buttons.B == ButtonState.Pressed && oldgPlayer1Plane.Buttons.B == ButtonState.Released)
                        {
                            menuselect = MenuScreen.plane;
                        }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            switch (menuselect)
            {
                ...
                case ((MenuScreen)1):
                    {
                        ...
                        spriteBatch.DrawString(Font1, "Team: " + T1R.ToString() + T1G.ToString() + T1B.ToString(), new Vector2(150, 250), Team1);
                        ...
                        spriteBatch.DrawString(Font1, "Team: " + T2R.ToString() + T2G.ToString() + T2B.ToString(), new Vector2(700, 250), Team2);
                ...
                case ((MenuScreen)2):
                    {
                        spriteBatch.Begin();
                        spriteBatch.Draw(Day, new Rectangle(405, 100, 270, 50), Color.Black);
                        spriteBatch.Draw(Dawn, new Rectangle(405, 250, 270, 50), Color.Black);
                        spriteBatch.Draw(Night, new Rectangle(405, 400, 270, 50), Color.Black);
                        spriteBatch.Draw(Dusk, new Rectangle(405, 550, 270, 50), Color.Black);
                        spriteBatch.DrawString(Font1, "Player 1 Pick Map To Begin", new Vector2(400, 650), Color.White);
                        spriteBatch.Draw(TeamSelect, MapSelector, Color.White);
                        spriteBatch.End();
                ...
                case ((MenuScreen)3):
                    {
                        spriteBatch.Begin();
                        for (int i = 0; i < Player1Plane.lives; i++)
                        {
                            spriteBatch.Draw(heartText, new Rectangle(45 * i, 0, 40, 40), Color.Firebrick);
                        }
                        for (int i = 0; i < Player2Plane.lives; i++)
                        {
                            spriteBatch.Draw(heartText, new Rectangle(1040 - 45 * i, 0, 40, 40), Color.Firebrick);
                        }
